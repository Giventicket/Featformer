...
def run(cfg: DictConfig) -> None:
    ...
    cfg.load_checkpoint_path = os.path.join(hydra.utils.get_original_cwd(), cfg.load_checkpoint_path)
    cfg.val_data_path = os.path.join(hydra.utils.get_original_cwd(), cfg.val_data_path)

    # build  TSPModel
    tsp_model = TSPModel(cfg)
    tsp_model = TSPModel.load_from_checkpoint(cfg.load_checkpoint_path)
    tsp_model.cfg = cfg
    ...
    dataset = TSPDataset(
        ...
        embedding_dim=cfg.embedding_dim,
        use_position_encoding=cfg.encoder_use_position_encoding,
        position_encoding_type=cfg.encoder_position_encoding_type,
    )
    ...
    trainer.validate(tsp_model, dataloaders=val_dataloader)
